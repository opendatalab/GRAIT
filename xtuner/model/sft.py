...
class SupervisedFinetune(BaseModel):
    ...
    def _split_for_sequence_parallel(data):
        ...
        # fixme 这里是随便写的测试语句（没过）
        ARGS_NEED_TO_SPLIT = ('input_ids', 'labels', 'position_ids', 'sample_alpha', 'sample_beta')
    ...
    def compute_loss(self, data, data_samples=None):
        # todo
        alpha = data.pop('alphas', None)
        beta = data.pop('betas', None)
        ...
        if get_sequence_parallel_world_size() > 1:
            ...
            outputs = self.llm(**data)
            loss_dict = {'loss': alpha * outputs.loss + beta}
            print("alpha_value", alpha)
            # print("alpha_value", alpha, "original loss: ", outputs.loss)
            # print("loss_dict: ", loss_dict['loss'])
            return loss_dict
